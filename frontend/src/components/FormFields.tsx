...
export function RegionSelector({ value, onChange, error }: { value: Array<Pick<Region, "code" | "name">>; onChange: (value: Array<Pick<Region, "code" | "name">>) => void; error?: string }) {
  ...
  return (
    <div className="field-group">
      ...
      {results.length > 0 && <ul className="search-results" aria-label="지역 검색 결과">{results.map((region) => <li key={region.code}><button type="button" onClick={() => { if (!value.some((item) => item.code === region.code)) onChange([...value, { code: region.code, name: region.name }]); setResults([]); setQuery(""); }}><strong>{region.name}</strong><span>{region.level === "SIDO" ? "시·도" : region.parentName}</span></button></li>)}</ul>}
